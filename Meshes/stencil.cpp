...
void Stencil::Compute_ipsi(bvp boundvalprob, double c2, char debug_fname[256]){
	if(pos_north.size() > 0){
		ipsi_north = Compute_ipsi(pos_north, boundvalprob, c2, debug_fname);
	}
	if(pos_south.size() > 0){
		ipsi_south = Compute_ipsi(pos_south, boundvalprob, c2, debug_fname);
	}
	if(pos_east.size() > 0){
		ipsi_east = Compute_ipsi(pos_east, boundvalprob, c2, debug_fname);
	}
	if(pos_west.size() > 0){
		ipsi_west = Compute_ipsi(pos_west, boundvalprob, c2, debug_fname);
	}
}
	...
	if(AreSame(X(1),stencil_parameters[1])){
		...
		for(unsigned int i = 0; i < G_south.size(); i++){
			...
			for(unsigned int j = 0; j < G_south.size(); j++){
				H_ij += ipsi_south(j,i)*boundvalprob.RBF(pos_south[j],X,c2);
			}
			G_south[i] += -H_ij*Y;
	...
	if(AreSame(X(1),stencil_parameters[3])){
		...
		for(unsigned int i = 0; i < G_north.size(); i++){
			...
			for(unsigned int j = 0; j < G_north.size(); j++){
				H_ij += ipsi_north(j,i)*boundvalprob.RBF(pos_north[j],X,c2);
			}
			G_north[i] += -H_ij*Y;
	...
	if(AreSame(X(0),stencil_parameters[0])){
		...
		for(unsigned int i = 0; i < G_west.size(); i++){
			...
			for(unsigned int j = 0; j < G_west.size(); j++){
				H_ij += ipsi_west(j,i)*boundvalprob.RBF(pos_west[j],X,c2);
			}
			G_west[i] += -H_ij*Y;
	...
	if(AreSame(X(0),stencil_parameters[2])){
		...
		for(unsigned int i = 0; i < G_east.size(); i++){
			...
			for(unsigned int j = 0; j < G_east.size(); j++){
				H_ij += ipsi_east(j,i)*boundvalprob.RBF(pos_east[j],X,c2);
			}
			G_east[i] += -H_ij*Y;
